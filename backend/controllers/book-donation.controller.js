...
const BookDonationController = {
	...
	async destroy(req, res, next) {
		try {
			...
			if (!donation) throw new ApiError(404, 'Book donation not found');
			if (!donation.tracking_id && donation.order_id) {
	try {
		const { data: order } = await DeliveryController.order(donation);

		const trackingId =
			order?.courier?.tracking_id ||
			order?.courier?.waybill_id ||
			order?.courier?.waybill_number ||
			order?.tracking_id;
		...
		console.log('Failed to sync tracking ID:', error.message);
	}
}
